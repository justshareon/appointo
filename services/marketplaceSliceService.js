...
const { recordOfferLog } = require('./offerLogService');
...
const GEO_SCOPE_TIER = ['local', 'town', 'city', 'state', 'All'];

function resolveTryScope(requestedScope, attempt, maxAttempts) {
  const req = String(requestedScope || 'All');
  if (req === 'All' || !GEO_SCOPE_TIER.includes(req)) return req;
  if (attempt >= maxAttempts) return 'All';
  const idx = GEO_SCOPE_TIER.indexOf(req);
  return GEO_SCOPE_TIER[Math.min(idx + attempt - 1, GEO_SCOPE_TIER.length - 1)];
}

function logSliceAttempt(result, opts, attempt, tryScope, requestedScope) {
  const itemCount = countSlice(result);
  recordOfferLog({
    level: itemCount ? 'L3' : (attempt >= MAX_SLICE_ATTEMPTS ? 'L1' : 'L2'),
    stage: 'slice',
    message: itemCount
      ? `Slice OK: ${itemCount} item(s) · scope=${tryScope} · cat=${opts.category} · type=${opts.type} · attempts=${attempt}`
      : `Slice EMPTY · scope=${tryScope} (req=${requestedScope}) · cat=${opts.category} · type=${opts.type} · attempts=${attempt}`,
    meta: {
      itemCount,
...
async function buildSliceBody(opts) {
  ...
  if (srcList.includes('deals')) {
    ...
    const rawDeals = await fetchDeals(safeLimit * 2);

    let filtered = filterRows(rawDeals, { category, type, scope, ctx, language });

    filtered = withinRecentDays(filtered, 21, ['updated_at', 'created_at', 'date', 'validity_to']);

    if (!filtered.length && rawDeals.length) {

      filtered = filterRows(rawDeals, { category, type, scope: 'All', ctx, language });
      ...
      recordOfferLog({
        ...
        message: `Date filter removed all ${rawDeals.length} deal(s) — using scope-All fallback`,

        meta: { scope, rawCount: rawDeals.length, category, type },
...
async function getSlice(opts = {}) {
  ...
  while (attempt < MAX_SLICE_ATTEMPTS) {
    ...
    const tryScope = resolveTryScope(requestedScope, attempt, MAX_SLICE_ATTEMPTS);
    ...
    lastResult.scope = requestedScope;



    logSliceAttempt(lastResult, {

      category, type, city, town, locality, state,

    }, attempt, tryScope, requestedScope);
    ...
    if (attempt < MAX_SLICE_ATTEMPTS) {
      ...
      LOG.info(`[Marketplace] Slice empty (attempt ${attempt}/${MAX_SLICE_ATTEMPTS}) scope=${tryScope} — widen geo`);
